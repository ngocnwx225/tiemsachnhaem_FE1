...
async function fetchCustomers() {
  try {
    const response = await fetch(
      "https://tiemsachnhaem-be-mu.vercel.app/api/users"
    );
    ...
    document.getElementById("customerList").innerHTML =
      '<tr><td colspan="5">Không thể tải dữ liệu khách hàng.</td></tr>';
  }
}
...
async function updateUserStatus(id, status) {
  try {
    const response = await fetch(
      `https://tiemsachnhaem-be-mu.vercel.app/api/users/${id}`,
      {
        method: "PUT",
...
function renderTable() {
  ...
  customers.reverse().forEach((customer) => {
    ...
    row.innerHTML = `
      ...
      <td><span class="status ${status ? "active" : "lock"}">${
      ...
      <td class="actions">
        <img src="https://img.icons8.com/material-outlined/24/000000/visible.png" alt="view" onclick="showPopup(this.parentNode.parentNode)">
         <img
          style="${user.id === customer.id && "cursor: not-allowed"}"
          src="https://img.icons8.com/material-outlined/24/${
            user.id === customer.id ? "808080" : status ? "ff0000" : "00ff00"
          }/${!status ? "unlock" : "lock"}.png"
          alt={!status ? "unblock" : "block"}
          onclick="showConfirmPopup('${
            user.id === customer.id
              ? null
              : customer.fullName
          }', this.parentNode.parentNode, ${!status})"
        />
      </td>
    `;
...
async function showPopup(row) {
  ...
  try {
    const response = await fetch(
      `https://tiemsachnhaem-be-mu.vercel.app/api/users/${customerId}`
    );
    ...
    document.getElementById("popupStatus").textContent = !status
      ? "Bị khóa"
      : "Hoạt động";
    document.getElementById("popupOrderCount").textContent =
      customer.totalOrders || 0;
    document.getElementById("popupTotalSpent").textContent =
      (customer.totalSpent || 0).toLocaleString("vi-VN") + "đ";
    document.getElementById("block-btn").textContent = !status
      ? "MỞ TÀI KHOẢN"
      : "KHÓA TÀI KHOẢN";
    document.getElementById("block-btn").style.backgroundColor = !status
      ? "#00ff00"
      : "#ff0000";
    ...
    const latestOrderDate =
      sortedOrders.length > 0
        ? new Date(sortedOrders[0].createdAt).toLocaleDateString("vi-VN")
        : "";
    ...
    document.getElementById("popupEarliestOrder").textContent =
      earliestOrderDate;
    ...
    if (customer.orders && customer.orders.length > 0) {
      customer.orders.forEach((order) => {
        ...
        row.innerHTML = `
          ...
          <td class="status-column"><span class="status ${
            order.status === "completed" ? "active" : "lock"
          }">${
...
async function confirmBlockAction() {
  if (currentRow) {
    ...
    if (updatedCustomer) {
      ...
      actionCell.src =
        "https://img.icons8.com/material-outlined/24/00ff00/unlock.png";
...
async function confirmUnlockAction() {
  if (currentRow) {
    ...
    if (updatedCustomer) {
      ...
      actionCell.src =
        "https://img.icons8.com/material-outlined/24/ff0000/lock.png";
...
[popupOverlay, confirmOverlay, unlockOverlay, successOverlay].forEach(
  (overlay) => {
    overlay.addEventListener("click", function (event) {
      if (event.target === overlay) {
        [closePopup, closeConfirmPopup, closeUnlockPopup, closeSuccessPopup][
          [
            "popupOverlay",
            "confirmOverlay",
            "unlockOverlay",
